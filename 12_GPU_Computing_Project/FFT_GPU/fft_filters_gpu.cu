// CUDA 2D FFT benchmark + filtering, using the SAME "common CUDA optimizations" style
// as the DFT-GPU version:
//
// (1) Transpose to avoid strided column access
//     - 2D FFT as: RowFFT -> Transpose -> RowFFT -> Transpose back
//
// (2) Tiled shared-memory transpose (32x32 + padding) for coalesced transpose
//
// (3) On-the-fly twiddle (sincos) in each FFT stage (no twiddle table)
//     - For each stage length len, twiddle = exp(sign*i*2π*j/len)
//
// (4) __restrict__ pointers in kernels
//
// (5) Fuse scale (1/(N*N)) + inverse shift (-1)^(x+y) into final output kernel
//
// Notes:
// - No cuFFT, no OpenCV.
// - This matches your CPU FFT math (radix-2 Cooley-Tukey), but GPU inverse scaling is
//   applied ONLY at the very end (1/(N*N)) in the output kernel (same final result).
// - N must be power of 2.
// - Spectrum saving matches your baseline: shift=true means DC already centered, so
//   spectrum_to_image(..., shift_center=false).
//
// Build:
...
ErrorStats compare_images(const Image& a, const Image& b) {
    if (a.w != b.w || a.h != b.h) { std::cerr << "compare_images: size mismatch\n"; std::exit(1); }
...
struct PerfRow { std::string test; int N; std::string stage; double ms; };
struct MetricsRow {
...
// ✅ FIX EMBEDDED HERE:
// Make bit_reverse callable from device code (kernel) AND host if you ever reuse it.
// Also keep it ABOVE kernels that call it.
...
// -------------------- GPU complex --------------------
struct cdouble2 { double x, y; }; // real, imag
__device__ __forceinline__ cdouble2 cadd(cdouble2 a, cdouble2 b){ return {a.x+b.x, a.y+b.y}; }
__device__ __forceinline__ cdouble2 csub(cdouble2 a, cdouble2 b){ return {a.x-b.x, a.y-b.y}; }
...
// Convert input image -> complex, applying shift (-1)^(x+y) if shift_on
...
// (2) Tiled transpose: classic 32x32 + padding
...
// Bit-reversal permutation for each row (in-place swaps), one thread per element
__global__ void bitreverse_rows_inplace(cdouble2* __restrict__ a, int n, int log2n)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x; // index in row
    int y = blockIdx.y * blockDim.y + threadIdx.y; // row
    if (x>=n || y>=n) return;

    unsigned int rx = bit_reverse((unsigned int)x, log2n);
    if ((unsigned int)x < rx) {
        int i = y*n + x;
        int j = y*n + (int)rx;
        cdouble2 tmp = a[i];
        a[i] = a[j];
...
// One FFT stage (radix-2 butterflies) per row, in-place.
// Each thread computes ONE butterfly (two outputs) for a specific stage length len.
// Total butterflies per row = n/2, so x dimension is [0, n/2).
//
// (3) On-the-fly twiddle via sincos. No twiddle table.
// (4) __restrict__ pointers.
__global__ void fft_stage_rows_inplace(cdouble2* __restrict__ a,
                                      int n, int len, int inverse)
{
    int b = blockIdx.x * blockDim.x + threadIdx.x; // butterfly index in [0, n/2)
    int y = blockIdx.y * blockDim.y + threadIdx.y; // row index
    if (y>=n) return;
    int half = len >> 1;
    if (b >= (n >> 1)) return;

    int group = b / half;       // which len-sized block
    int j     = b - group*half; // position within half
    int base  = group * len;
    ...
    double ang  = sign * (2.0 * PI * (double)j / (double)len);

    double sn, cs;
    sincos(ang, &sn, &cs);
    cdouble2 w{cs, sn}; // exp(i*ang)
...
// Elementwise mask multiply: G = F * H
...
// (5) Fused final pass: real + (1/(N*N)) scale + undo shift
...
struct GpuContext {
    ...
    // Helper: run row FFT in-place on buffer a (size n x n)
    void row_fft_inplace(cdouble2* a, int inverse) {
        // bit reversal per row
        dim3 blk_br(16,16);
        dim3 grd_br((n+blk_br.x-1)/blk_br.x, (n+blk_br.y-1)/blk_br.y);
        bitreverse_rows_inplace<<<grd_br, blk_br, 0, stream>>>(a, n, log2n);

        // stages
        dim3 blk_stage(256, 1);
        dim3 grd_stage(((n/2) + blk_stage.x - 1)/blk_stage.x, n);
        for(int len=2; len<=n; len<<=1) {
            fft_stage_rows_inplace<<<grd_stage, blk_stage, 0, stream>>>(a, n, len, inverse);
        }
    }

    // (1)+(2)+(3)+(4)
    void fft2_forward_shift(const Image& img) {
        ...
        // row FFT on d_c0
        row_fft_inplace(d_c0, /*inverse=*/0);
        ...
        // row FFT on transposed
        row_fft_inplace(d_c2, /*inverse=*/0);

        // transpose back -> d_c0
        transpose_cdouble2<32,8><<<grdT, blkT, 0, stream>>>(d_c2, d_c0, n);
    ...
    // (1)+(2)+(3)+(4)+(5)
    void ifft2_inverse_shift_from_G() {
        dim3 blkT(32,8);
        dim3 grdT((n+32-1)/32, (n+32-1)/32);
        ...
        // row inverse FFT on d_c2
        row_fft_inplace(d_c2, /*inverse=*/1);

        // transpose back -> d_c0
        transpose_cdouble2<32,8><<<grdT, blkT, 0, stream>>>(d_c2, d_c0, n);

        // row inverse FFT on d_c0
        row_fft_inplace(d_c0, /*inverse=*/1);

        // fused final: scale 1/(N*N) + inverse shift
        dim3 blk_out(16,16);
...
int main(int argc, char** argv) {
    ...
    if (N <= 0 || !is_power_of_two(N)) {
        std::cerr << "Invalid N: " << N << " (must be power of 2)\n";
        return 1;
    }

    std::cout << "CUDA COMMON-OPT FFT benchmark N=" << N
    ...
    auto time_fft2 = [&](const Image& img)->double{
        return avg_ms_cuda(runs, [&]{ gpu.fft2_forward_shift(img); }, gpu.stream);
    };
    auto time_mask = [&](const std::vector<double>& H)->double{
        return avg_ms_cuda(runs, [&]{ gpu.apply_mask_to_F(H); }, gpu.stream);
    };
    auto time_ifft2 = [&]()->double{
        return avg_ms_cuda(runs, [&]{ gpu.ifft2_inverse_shift_from_G(); }, gpu.stream);
    };
    ...
    // ---------------- E: Periodic noise -> auto notch denoise (auto only) ----------------
    {
        ...
        // Notch build stays CPU-side for baseline-identical behavior
